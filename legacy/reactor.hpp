...
// =============================================================================
// 已废弃：本文件属于"主/从 Reactor + 手写 epoll"时代的实现，
// 已被 Boost.Asio 传输层取代（见 ../asio_transport.hpp 与 ../http_session.hpp）。
// 保留仅供对照阅读，**不参与编译**（不在 CMakeLists / vcxproj 的源列表里）。
// =============================================================================
...
namespace project {
     ...
     *
     * \deprecated 已被 Asio 的"每连接一个会话对象"模型取代：socket 建于独立 strand，
     *             回调天然串行，不再需要 fd_contexts_ 表与 task_in_flight_ 原子。
     *             请改用 ../asio_transport.hpp 与 ../http_session.hpp。
     */
    class [[deprecated("SubReactor 已被 Boost.Asio 传输层取代，请使用 asio_transport.hpp / http_session.hpp")]] SubReactor {
    private:
     ...
     *
     * \deprecated 已被 AsioTransport 的 accept 循环取代：acceptor 与 io_context 同属一个
     *             事件循环，新连接直接交给 HttpSession，不再需要"轮询选子 Reactor + 二次 post"。
     *             请改用 ../asio_transport.hpp。
     */
    class [[deprecated("MainReactor 已被 AsioTransport 取代，请使用 asio_transport.hpp")]] MainReactor {
    public:
        /**
