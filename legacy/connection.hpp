...
// =============================================================================
// 已废弃：fd 绑定的连接容器（读写缓冲用互斥锁保护，另用 task_in_flight_ /
// data_while_busy_ 原子做连接级串行化，并以 fd 编号作为连接身份）。
// 已被 HttpSession<Stream> 取代：会话对象即连接身份，socket 建于独立 strand，
// 回调天然串行，成员无需加锁。
// 保留仅供对照阅读，**不参与编译**。
// =============================================================================
 ...
 * 读缓冲区与写缓冲区均通过内部互斥锁保护
 *
 * \deprecated 请改用 ../http_session.hpp 的 HttpSession<Stream>。
 */
class [[deprecated("Connection 已被 HttpSession<Stream> 取代，请使用 http_session.hpp")]] Connection{
public:
    /**
