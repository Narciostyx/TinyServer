...
## 核心依赖流
1. `Server` -> `Config`, `Acceptor`, `MainReactor`, `SubReactor`, `ThreadPool`, `ConnPool` (隐式单例), `Log` (隐式单例)
...
1. **启动阶段**：
   ...
   - `Server::init()` 加载配置，启动基础组件(`ConnPool` / `Log`)。创建主次`Reactor`并设定挂载逻辑。
...
3. **数据读写流转**：
   ...
   - 这避免了阻塞 `Reactor`，`Server` 紧接着将 HTTP 解析、业务逻辑路由与回复数据 `send` 等阻塞操作，封装成 `Lambda` 包装后，推送到 `ThreadPool::enqueue()`。
4. **清理机制**：
